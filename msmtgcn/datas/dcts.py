...
def dct_transform_paddle(data, dct_m, dct_n):
    '''
    B, 60, 35
    '''
    batch_size, features, seq_len = data.shape
    ...
    out_data = paddle.matmul(dct_m[:dct_n, :], data)  # [dct_n, 180077*60]
    out_data = out_data.transpose((1, 0)).reshape((-1, features, dct_n))  # [b, 60, dct_n]
    return out_data

def reverse_dct_paddle(dct_data, idct_m, seq_len):
    '''
    B, 60, 35
    '''
    batch_size, features, dct_n = dct_data.shape
    ...
    dct_data = dct_data.transpose((2, 0, 1)).reshape((dct_n, -1))  # dct_n, B*60
    out_data = paddle.matmul(idct_m[:, :dct_n], dct_data).reshape((seq_len, batch_size, -1)).transpose((1, 2, 0))
    return out_data